...
local function launch_master()
    ...
    function CMD_MASTER.open(source, conf)
        ...
        assert(instance > 0)
        local balance = 1
        -- print("开启server_name",SERVICE_NAME)
        for i=1,instance do
...
local function launch_slave()
    local max_login_num = skynet.getenv("loginserver_num")
    local login_state = {}
    ...
    function handler.connect(fd)
        ...
        connection[fd] = c
        skynet.send(watchdog, "lua", "socket", "open", fd, addr, skynet.self(),usermap[fd])
    end
    ...
    function handler.handshake(fd, header, url)
        ...
        local ip = addr:match("(.+):(.+)")
        header.ip = ip
        local ok,pid = skynet.call(".login_" .. math.random(1,max_login_num),"lua","auth",header)
        if not ok then
            error("握手失败，账号密码错误")
        end
        usermap[fd] = {pid = pid,account = header.account , password = header.password, ip = ip,channel = header.channel}
        skynet.error("登录成功",pid)
        return true
    end

    function handler.message(fd, msg)
        -- skynet.error("ws gate 收到消息: " .. tostring(fd), "长度：",#msg)
